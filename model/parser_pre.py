...
@Registrable.register('parser_pre')
class ParserPre(nn.Module):
    ...
    def __init__(self, args, vocab, transition_system):
        ...
        self.branch_selector = BranchSelector(args.hidden_size, args.action_embed_size, args.field_embed_size,
                                             len(transition_system.grammar), len(transition_system.grammar.fields))
        self.max_field = 6
    ...
    def score(self, examples, epsilon=0.0):
        ...
        query_vectors, new_batch = self.decode_reinforce(batch, src_encodings, dec_init_vec, epsilon=epsilon)
        
        #calculate rewards
        select_action_prob = self.baseline_classifier(src_encodings, query_vectors, new_batch)
        losses = torch.sum(select_action_prob, dim=0)
        return [losses]
    ...
    def decode_reinforce(self, batch, src_encodings, dec_init_vec, epsilon):
        ...
        Returns:
            1. Query vectors, a variable of shape (tgt_action_len, batch_size, hidden_size)
            2. Reward span, a list of shape (batch_size, tgt_action_len + 1)
            3. policy_prob, a variable of shape (batch_size, tgt_action_len, max_field)
            4. policy_prob_mask, a variable of shape (batch_size, tgt_action_len, max_field)
            5. new_batch, with target actions in sample order
        """       
        batch_size = len(batch)
        args = self.args
        batch = copy.deepcopy(batch)
        # (batch_size, )
        # new_tgt_actions = [copy.deepcopy(e.tgt_actions) for e in batch.examples]
        ...
        policy_prob_mask = []
        policy_prob = []
        sample_action_prob = [{} for _ in range(batch_size)]
        ...
        for t in range(batch.max_action_num):
            ...
            for batch_id, e in enumerate(batch.examples):
                ...
                if t < len(e.tgt_actions) and isinstance(e.tgt_actions[t].action, ApplyRuleAction) \
                    ...
                    #if not sample then in the same span id
            ...
            if masks:
                ...
                with torch.no_grad():
                    # infer the branch
                    pred_branch, pred_prob = self.branch_selector.sample(att_t.data, action_embs.data, fields_embs.data, masks.data, select=False, epsilon=epsilon)
