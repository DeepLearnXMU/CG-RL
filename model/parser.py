        ...
        while len(completed_hypotheses) < beam_size and t < args.decode_max_time_step:
            ...
            for new_hyp_score, new_hyp_pos in zip(top_new_hyp_scores.data.cpu(), top_new_hyp_pos.data.cpu()):
                ...
                if isinstance(action, ApplyRuleAction) and len(action.production.fields) > 1:
                    if branch_mode == 'save':
                        # maybe some idx not saved
                        if example.idx in self.data_pseudo_label.keys():
                            label = self.data_pseudo_label[example.idx]
                            record=-1
                            for index, a in enumerate(example.tgt_actions):
                                if a.action == action and a.frontier_prod == action_info.frontier_prod:
                                    record=index
                                    break
                            if record != -1 and len(label[record]) > 1:
                                assert len(example.tgt_actions[record].action.production.fields) == len(label[record])
                                shuffle_id = sorted(range(len(label[record])), key=lambda k: -label[record][k])
                                #print(action)
                                #print(shuffle_id)
                                #print(label[record])
                                assert len(action.production.fields) == len(label[record])
                    elif branch_mode == 'reverse':
                        shuffle_id = [i for i in range(len(action.production.fields))]
                        shuffle_id.reverse()
                    elif branch_mode == 'random':
                        shuffle_id = [i for i in range(len(action.production.fields))]
                        random.shuffle(shuffle_id)
                    elif branch_mode == 'second':
                        shuffle_id = [i for i in range(len(action.production.fields))]
                        shuffle_id.reverse()
                        first = shuffle_id.pop(0)
                        shuffle_id.append(first)
                    else:
                        shuffle=None
